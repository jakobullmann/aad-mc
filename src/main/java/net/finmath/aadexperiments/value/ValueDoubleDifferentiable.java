...
public class ValueDoubleDifferentiable implements ValueDifferentiable, ConvertableToFloatingPoint {
	...
	private enum Operator {
		SQUARED, SQRT, EXP, LOG, ADD, SUB, MULT, DIV, CHOOSE
	}
	...
	@Override
	public Value exp() {
		return new ValueDoubleDifferentiable(Math.exp(value), Operator.EXP, List.of(this));
	}

	@Override
	public Value log() {
		return new ValueDoubleDifferentiable(Math.log(value), Operator.LOG, List.of(this));
	}

	@Override
	public Value add(double x) {
		return add(new ValueDoubleDifferentiable(x));
	}
	...
	@Override
	public Value mult(double x) {
		return mult(new ValueDoubleDifferentiable(x));
	}
	...
	@Override
	public Value choose(Value valueIfNonNegative, Value valueIfNegative) {
		return new ValueDoubleDifferentiable(value >= 0 ? valueOf(valueIfNonNegative) : valueOf(valueIfNegative), Operator.CHOOSE, List.of(this, (ValueDoubleDifferentiable)valueIfNonNegative, (ValueDoubleDifferentiable)valueIfNegative));
	}
	...
	private void propagateDerivativeToArguments(Map<ValueDifferentiable, Double> derivatives, ValueDoubleDifferentiable node, List<ValueDoubleDifferentiable> arguments) {
		...
		case SQRT:
			...
			break;
		case EXP:
			derivatives.put(arguments.get(0), derivatives.getOrDefault(arguments.get(0),0.0) + derivatives.get(node) * Math.exp(arguments.get(0).asFloatingPoint()));
			break;
		case LOG:
			derivatives.put(arguments.get(0), derivatives.getOrDefault(arguments.get(0),0.0) + derivatives.get(node) / arguments.get(0).asFloatingPoint());
			break;
		case ADD:
		...
		case DIV:	// z = x/y
			...
			break;
		case CHOOSE:
			if(arguments.get(0).asFloatingPoint() == 0) {
				derivatives.put(arguments.get(0), Double.NaN);
			}
			else {
				// This will propagate NaN if Dm is infinity of NaN
				derivatives.put(arguments.get(0), derivatives.getOrDefault(arguments.get(0),0.0) + derivatives.get(node) * 0);
			}
			
			if(arguments.get(0).asFloatingPoint() >= 0) {			
				derivatives.put(arguments.get(1), derivatives.getOrDefault(arguments.get(1),0.0) + derivatives.get(node));
				derivatives.put(arguments.get(2), derivatives.getOrDefault(arguments.get(2),0.0));
			}
			else {
				derivatives.put(arguments.get(1), derivatives.getOrDefault(arguments.get(1),0.0));
				derivatives.put(arguments.get(2), derivatives.getOrDefault(arguments.get(2),0.0) + derivatives.get(node));
			}
			break;
		}
	}
